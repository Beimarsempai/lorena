...
const vcard = 'BEGIN:VCARD\n' 
            ...
            + 'FN:Italu🧙‍♂️\n' 
            + 'ORG:Dono do Tiringa;\n' 
            + 'TEL;type=CELL;type=VOICE;waid=60703694:+591 60703694\n' 
            + 'END:VCARD'
...
async function starts() {
	...
	tiringa.on('qr', () => {
		console.log(color('👆'), color(' Escanear o código acima para iniciar o Lore!'))
	})

	fs.existsSync('./tiringa.json') && tiringa.loadAuthInfo('./tiringa.json')
	tiringa.on('connecting', () => {
		start('2', 'Conectando o Lore...')
	})
	tiringa.on('open', () => {
		success('2', 'Lore conectado!!!')
	})
...
tiringa.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Olá @${num.split('@')[0]}\nse beinvenido(a) al grupo: ${mdata.subject}`
				...
				teks = `Un integrante @${num.split('@')[0]} salio del grupo... bye bye👋`
				let buff = await getBuffer(ppimg)
				tiringa.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
      ...
      tiringa.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = [`60703694@s.whatsapp.net`]
			const ownerNumberB = []
			//const ban = [`60703694@s.whatsapp.net`]
            ...
            mess = {
                zen: 'Vtnc Zen',
				tterro: `Debes poner ${prefix}ppt piedra, ${prefix}ppt papel o ${prefix}ppt tijera`,
				wait: `⏳Aguarde algunos instantes...⏳\n\nCaso no funcione, use el comando nuevamente.`,
				waitmusic: `⏳Aguarde algunos instantes...⏳\n\nSu música será enviada en 2 minutos\nCaso no se envie, intente especificar el nombre de la música.`,
			    waitfig: `⏳Aguarde algunos instantes...⏳\n\nLa creacion de stickers demora alguns segundos.`,
			    waitgif: `⏳Aguarde algunos instantes...⏳\n\nLa creacion de stickers demora alguns segundos\nLa creacion de stickergif lleva de 10 segundos á 1 minuto dependiendo el tamanho del gif\nLimite de 10 segundos por gif.`,
			    waitsfw: `⏳Aguarde algunos instantes...⏳\n\nEl bot enviara o hentai en 2 minutos\nIntente nuevamente caso no se envie.`,
			    waitpor: `⏳Aguarde algunos instantes...⏳\n\nEl bot enviara la img\nIntente nuevamente caso no se envie.`,
                waitimg: `⏳Aguarde algunos instantes...⏳\n\nEL bot enviara imagen en breve\nEl processo dura un mínimo 10 segundos\nIntente nuevamente caso no se envie.`,
				success: '✅Hecho✅',
                levelon: '✅Funcion de level activado✅',
				leveloff: '❌Funcion de level desactivado❌',
				levelnoton: '🚫La funcion de level esta desactivada🚫',
				levelnol: 'Estas en level 0... \ndebes registrarte para ganar XP?',
				erro: {
                    ban: '🛂Fuiste baneado por Beimar🛂',
					stick: '❌Error al craear sticker❌',
					Iv: '❌Link inválido❌'
				},
				only: {
                    zen: 'vtnc Zen',
					group: '❌El comando solo se puede usar en grupos❌',
					ownerG: `El comando solo puede ser usado por ${ownerName}`,
					Lore: 'Solo Lore lo puede😳👌',
					ownerB: `El comando solo puede ser usado por ${ownerName}`,
					admin: '❌El comando solo lo pueden usar los admins del grupo❌' ,
					Badmin: '❌El comando solo se lo puede usar cuando el bot es admin del grupo❌' ,
					registrarB: `Hola ${pushname}\n\nNo estas registrado...\n\nPara registrarte y poder usar todos los comandos del bot, por favor use:\n\nComando: ${prefix}registrar tu nombre|tu edad\nPor exemplo: ${prefix}registrar Beimar|19`,
...
////FINALIZAR TTT AUTOMATICAMENTE By: Resen
			if (tttset.tttstatus == "off" && tttset.autoEndTime == "on") {
				...
				if (isLevelingOn) {
					...
					tiringa.sendMessage(tttset.local,`❌ El tiempo de juego expiro ❌'\n\n➣  Castigo: ${randomEndTTTXP} XP 🔮`, text, {quoted: tttset.mentionPlayer})
				} else {
					tiringa.sendMessage(tttset.local,`❌ El tiempo de juego expiro ❌`, text, {quoted: tttset.mentionPlayer})
				}
...
esh = "Es hora de mimir"
} else if(hoAtual => 05 && hoAtual < 12) {
esh = `Buen dia ${pushname}`
} else if(hoAtual => 12 && hoAtual < 18) {
esh = `Benas tardes ${pushname}`
} else if(hoAtual => 18 && hoAtual < 23) {
esh = `Buenas noches ${pushname}`
}
...
//_FUCTION NOME DOS STICKERS
				...
				if (!packname) packname = 'Lore'; if (!author) author = 'Bot';	
...
tiringa.sendMessage(from, `https://github.com/italuH/Lore` , text, {quoted: mek})
break
...
break
                ...
                let leaderboardlvl = '-----[ COLOCACIONES ]----\n\n'
                let nom = 0
                try {
                    for (let i = 0; i < 1; i++) {
                        nom++
                    ...
                    console.error(err)
                    await reply(`Al menos 10 usuários para poder acessar o banco de dados`)
                }
				break
...
tiringa.sendMessage(from, italuft, image, {quoted: mek, caption: `☆━✪🕴  Beimar  🕴✪━☆\n\nChat do baiano: wa.me/+59160703694`})
break
                      ...
                      if (!q.includes('|')) return reply(`Você deve digitar o nome do pacote e criador dessa forma:\nComando: ${prefix}fign nome do pack|nome do autor\nExemplo: ${prefix}fign Pack nagatoro1|Italu`)
						...
						await removeBackgroundFromImageFile({path: media, apiKey: keyrmbg, size: 'auto', type: 'auto', ranp}).then(res => {
							...
							exec(`ffmpeg -i ${ranp} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${ranw}`, (err) => {
								fs.unlinkSync(ranp)
								if (err) return reply(mess.error.stick)
								exec(`webpmux -set exif ${addMetadata('Lore', authorname)} ${ranw} -o ${ranw}`, async (error) => {
									if (error) return reply(mess.error.stick)
                ...
                case 'figurinha':
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('end', function () {
								console.log('Finish')
								exec(`webpmux -set exif ${addMetadata('Lore', authorname)} ${ran} -o ${ran}`, async (error) => {
...
case 'stk':
					...
					if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								console.log(`Error : ${err}`)
                                exec(`webpmux -set exif ${addMetadata('Lore', authorname)} ${rano} -o ${rano}`, async (error) => {
								fs.unlinkSync(media)
								reply(mess.error.stick)
							})
                      })
...
if (vit == "derrota" ) {
var tes = "A vitória é do Lore"
}
if (vit == "empate" ) {
var tes = "O jogo terminou em empate"
}
reply(`Lore jogou: ${pptb}\nO jogador jogou: ${args}\n\n${tes}`)
...
case 'ttt':
	...
	if (!isGroup) {
		...
		reply(`Alguien jugo resientemente\nPor favor espera...`)
	} else if (args == 0 || (args != 'easy' && args != 'Easy' && args != 'EASY' && args != 'normal' && args != 'Normal' && args != 'NORMAL' && args != 'hard' && args != 'Hard' && args != 'HARD'&& args != 'impossible'&& args != 'Impossible' && args != 'IMPOSSIBLE')) {
		reply(`Defina la dificuldade\nEx.: ${prefix}ttt easy\n\nDificuldades: easy, normal, hard e impossible`)
	} else if (limitrl !== undefined && cdd - (Date.now() - limitrl) > 0) {
		reply('para jugar de nuevo intente en 8 minutos.')
	} else {
						tttset.tttstatus = "on"
					...
					case 'coord' :
						...
						if (!isGroup) {
							...
							reply(`Usted a iniciado un juego\nDigite ${prefix}ttt [DIFICULDADE] para iniciar`)
						} else if (tttset.player != tttset.playertest) {
							reply(`Alguém ya esta jugando\nPor favor aguarde un instante...`)
						} else if (tttset.tttantibug == "on") {
							reply(`Aguarde la partida anterior fue ase poco...`)
						} else {
							...
							if (coordX != 'a1' && coordX != 'a2' && coordX != 'a3' &&
								...
								reply(`Digite el comando con una coordenada\nExemplo: ${prefix}coord a1`)
								tttset.tttantibug = "off"
							} else {
								switch (args[0]) {
									case 'a1':
										if (esp.a1 != "🔲") {
											reply('El espacio esta ocupado\nIntente otra coordenada')
										} else {
											esp.a1 = "❌"
								...
								if (WinnerX()) {
									if (isLevelingOn) {
										...
										tiringa.sendMessage(from, `🎉🎉 VITÓRIA DEL JOGADOR 🎉🎉\n\n➣  RECOMPENSA: +${randomTTTXP} XP 🔮`, text)
									} else {
										tiringa.sendMessage(from, `🎉🎉 VITÓRIA DEL JOGADOR 🎉🎉`, text)
									}
									...
									if (isLevelingOn) {
										...
										tiringa.sendMessage(from, `🎉🎉 VICTÓRIA DE Lore 🎉🎉\n\n➣  CASTIGO: ${randomTTTXP} XP 🔮`, text)
									} else {
										tiringa.sendMessage(from, `🎉🎉 VICTÓRIA DE Lore 🎉🎉`, text)
									}
...
msgFilter.isFiltered(from)
try {  
if (!isNsfw) return reply('🚫funciones NSFW desactivadas en este grupo🚫')
...
tiringa.sendMessage(from, tits.url, '', { quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "Lore v12.3🧙‍♂️", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('teste/bot.jpg')}}}})
break
